...
|---|---|---|
| `vision_bundle.mjs`, `vision_wasm_internal.{js,wasm}` | ~12 MB | **Not committed, not bundled.** Copied out of `node_modules` by `make seg-assets` and by `vercel.json`'s build command, served from Voxal's own origin, fetched lazily on first use. |
