...
window.addEventListener('DOMContentLoaded', function() {
  ...
  function initAboutSection(versionElId, dateElId) {
    ...
    // build-info.js is generated by `make build-web` for the web deploy only
    // (never present on Tauri/Capacitor) — when it's there, show what was
    // actually deployed instead of the desktop release version/date.
    if (typeof window.VOXAL_COMMIT !== 'undefined') {
      var webBuildDate = new Date(window.VOXAL_WEB_BUILD_DATE).toLocaleDateString('en-GB', {
        day: 'numeric', month: 'short', year: 'numeric'
      });
      versionEl.textContent = window.VOXAL_COMMIT;
      dateEl.textContent = webBuildDate;
      return;
    }
