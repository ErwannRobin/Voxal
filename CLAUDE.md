...
| `blur-strength` | `VideoEffects.STRENGTH_KEY` | Blur radius as a fraction of the frame's long side, `0.02`–`0.20` (default `0.08`, stored as an absence). Settings → Advanced → Background blur strength. Applied live by `VideoEffects.setStrength()` — a uniform change, so it never swaps a track. Declared only in `video-effects.js`, same rule as above |
