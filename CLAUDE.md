...
`scripts/coverage-report.mjs` folds all three into one markdown table (`make coverage-summary`). Every source is optional — a suite that was not run is reported as such rather than failing — so the same script serves a partial local run and CI. With `--write-badge` (`make coverage-badge`) it also rewrites the README badge between its `<!-- coverage-badge -->` markers, from `main.js`'s own line coverage. That target re-measures first, so the badge cannot be published off a stale report, and it fails loudly rather than no-op'ing when there is nothing to read.

**In CI** (`.github/workflows/tests.yml`), the `coverage` job runs all three, puts the table in the run summary and uploads the HTML as an artifact. It is `continue-on-error`, and so is each measurement leg inside it — coverage is information for the reviewer, never a merge gate, so it stays out of `all-tests-green`'s `needs:`. **It writes nothing back to the repository** — the README badge is refreshed by hand with `make coverage-badge`, never by CI. It was a CI step once and could not be: `main`'s ruleset means the change has to arrive as a pull request, and a workflow's own `GITHUB_TOKEN` can neither open one (without a repository setting) nor produce one that is mergeable (the required check never reports on it). See `KNOWLEDGE/learning.md`. So the badge is as fresh as the last person to run the target — refresh it before a release, or whenever the number has visibly moved.
...
- The MediaPipe segmentation runtime (`src/assets/seg/vision_*`, ~12 MB) is the one exception to "everything is vendored": it is gitignored and staged out of `node_modules` by `seg-assets.sh` (run by `make seg-assets`, the desktop build targets, and the Vercel deploy's `vercel-build.sh`). Bundled into the Tauri app and served same-origin on web; `make cap-sync` strips it from the mobile bundles, which fetch it lazily from `web.voxal.app` — never from `presenceBase()`, which is the presence API. `make cap-sync` strips it from the mobile bundles.
...
| `seg-assets-url` | `VideoEffects` internal | Override for where the background-effects WASM runtime is fetched from (default: same-origin on web/Tauri, `https://web.voxal.app/assets/seg/` on mobile); self-host/test only |
