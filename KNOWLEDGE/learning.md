...
- **WebRTC can't move between windows**: pop-out opens the new window, then the embed calls `leaveRoom()`. `window.open` must run synchronously in the click handler (user-activation) or it's popup-blocked; if it returns `null` (e.g. iframe `sandbox` without `allow-popups`) we keep the session and emit `{type:'popout-blocked'}` to the parent.
- **The parent page can't watch the pop-out window** — `window.open` returns the `WindowProxy` to the *iframe* that called it, not to the parent, and the parent has no way to obtain it. So the iframe polls `win.closed` (`watchPopoutWindow`, ~1s interval — `closed` is one of the few cross-origin-readable props) and relays `{type:'popout-closed'}` to the parent. There's no DOM "close" event on an opened window; polling is the only option. The pop-out button is **opt-in**: it only appears when `IS_TINY_EMBED && _isIframe && ALLOW_POPOUT` (`body.embed-can-popout`), hidden in `tiny-micro` (<100 px). `ALLOW_POPOUT` is enabled by adding `?popout=1` (aliases `allowPopout` / `canPopout`) to the iframe `src`.
