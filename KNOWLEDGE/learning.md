...
- **Host heartbeat**: the host must broadcast a periodic heartbeat over the host `DataConnection`, and non-host peers must trigger migration if that heartbeat stays stale past the timeout. PeerJS may not emit a prompt `close` when the host dies silently.
